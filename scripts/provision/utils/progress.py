...
class ProgressTracker:
    ...
    def get_overall_progress(self) -> Dict:
        ...
        completed = sum(1 for d in self.downloads.values() if d['status'] in ['completed', 'failed'])
        succeeded = sum(1 for d in self.downloads.values() if d['status'] == 'completed')
        ...
        total_size = sum(d.get('total_size', 0) for d in self.downloads.values() if d.get('total_size'))
    ...
    def get_status_summary(self) -> str:
        ...
        lines.append(f"Success: {progress['succeeded']}, Failed: {progress['failed']}, Active: {progress['active']}")
        
        if progress['total_size_mb'] > 0:
            lines.append(f"Downloaded: {progress['downloaded_mb']:.1f}MB / {progress['total_size_mb']:.1f}MB ({progress['percentage']}%)")
