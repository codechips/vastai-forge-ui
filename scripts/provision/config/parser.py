...
class ConfigParser:
    ...
    def _validate_model_config(self, model_name: str, model_config: Any, category: str) -> None:
        """
        ...
        if isinstance(model_config, str):
            ...
            if not (model_config.startswith('http://') or model_config.startswith('https://')):
                raise Exception(f"Model '{model_name}' URL must start with http:// or https://")
            return
    ...
    def _validate_hf_config(self, model_name: str, config: Dict[str, Any]) -> None:
        ...
        if 'repo' not in config:
            raise Exception(f"HuggingFace model '{model_name}' missing required 'repo' field")
        
        repo = config['repo']
        if not isinstance(repo, str) or '/' not in repo:
            raise Exception(f"HuggingFace model '{model_name}' 'repo' must be in format 'user/repo'")
    ...
    def _validate_civitai_config(self, model_name: str, config: Dict[str, Any]) -> None:
        ...
        if 'model_id' not in config:
            raise Exception(f"CivitAI model '{model_name}' missing required 'model_id' field")
        
        model_id = config['model_id']
        if not isinstance(model_id, (str, int)):
            raise Exception(f"CivitAI model '{model_name}' 'model_id' must be string or integer")
    ...
    def _validate_url_config(self, model_name: str, config: Dict[str, Any]) -> None:
        ...
        if not (url.startswith('http://') or url.startswith('https://')):
            raise Exception(f"URL model '{model_name}' 'url' must start with http:// or https://")
