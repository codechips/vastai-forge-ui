...
class ProvisioningSystem:
    ...
    def _log_gated_model_error(self, model: Dict, platform: str) -> None:
        ...
        self.logger.error(f"Model: {model.get('repo', model.get('model_id', 'unknown'))}")
        self.logger.error(f"Platform: {platform}")
        self.logger.error(f"Reason: Terms of Service not accepted or invalid token")
        
        if platform == 'HuggingFace':
            repo = model.get('repo', '')
            if repo:
                self.logger.error(f"Action: Visit https://huggingface.co/{repo} and click 'Agree and access repository'")
    ...
    def _create_download_task(self, model_name: str, model_config: Dict, target_dir: Path):
        ...
        if source == 'huggingface':
            return self._download_huggingface_model(model_name, model_config, target_dir)
        elif source == 'civitai':
    ...
    async def _download_huggingface_model(self, model_name: str, config: Dict, target_dir: Path) -> bool:
    ...
    async def _download_civitai_model(self, model_name: str, config: Dict, target_dir: Path) -> bool:
    ...
    async def _download_direct_url(self, model_name: str, config, target_dir: Path) -> bool:
