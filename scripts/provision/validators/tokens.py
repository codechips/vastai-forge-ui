...
class TokenValidator:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.info(f"HuggingFace token valid for user: {username}")
                        return True
                    else:
                        self.logger.error(f"HuggingFace token validation failed: HTTP {response.status}")
                        return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.error(f"CivitAI token validation failed: HTTP {response.status}")
                        return False
    ...
    async def validate_all_tokens(self) -> Dict[str, bool]:
        ...
        results = {
            'huggingface': hf_valid,
            'civitai': civitai_valid
        }
        
        # Log summary
        valid_count = sum(results.values())
        total_count = len([token for token in [self.hf_token, self.civitai_token] if token])
    ...
    async def check_repo_access(self, repo_id: str, platform: str = "huggingface") -> bool:
        """
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(f"Unexpected status {response.status} for {repo_id}")
                        return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        self.logger.warning(f"Unexpected status {response.status} for model {model_id}")
                        return False
